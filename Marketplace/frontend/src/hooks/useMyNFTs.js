...
export function useMyNFTs({ address, walletProvider, isConnected, showError }) {
  ...
  const loadMyNFTs = useCallback(
    async (accountOverride) => {
      const targetAddress = accountOverride ?? address;
      const allowWithoutConnection = Boolean(accountOverride);
      if (
        (!isConnected && !allowWithoutConnection) ||
        !targetAddress ||
        !walletProvider
      )
        return;
      ...
      try {
        ...
        const balanceBN = await withRetry(() => nft.balanceOf(targetAddress));
        ...
        if (balance === 0) {
          setMyNFTs([]);
          try {
            const proceeds = await withRetry(() =>
              market.getProceeds(targetAddress)
            );
        ...
        for (let i = 0; i < balance; i++) {
          try {
            const id = await withRetry(
              () => nft.tokenOfOwnerByIndex(targetAddress, i),
